...
@Component({
  ...
  templateUrl: './location-selector.template.html',
  styles: [`
  .location-selector {
    width: 100%;
...
// ]
  ...
  public onClick(event, organization, company, businessUnit){
    ...
    } else
    {
      ...
      if(organization&&company) {
          if (!company.checked) { 
            ...
            if(this.source == 'provider' || this.source == 'library'){
	            company.businessUnits.forEach((businessUnit, index) => {
	              if (this.selectedLocations.indexOf(businessUnit._id)<0) {
	                this.selectedLocations.push(businessUnit._id);
    ...
    // console.log(this.selectedLocations, 'selectedLocations')
